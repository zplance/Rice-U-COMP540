...
def select_lambda_crossval(X,y,lambda_low,lambda_high,lambda_step,penalty):
    ...
    for lam in np.linspace(lambda_low, lambda_high, np.abs(lambda_high - lambda_low) / lambda_step + 1):
        if (penalty == 'l1'):
            lreg = linear_model.LogisticRegression(penalty = penalty, solver = 'liblinear', C = 1/lam, fit_intercept = True)
        if (penalty == 'l2'):
            lreg = linear_model.LogisticRegression(penalty = penalty, solver = 'lbfgs', C = 1/lam, fit_intercept = True)
        model_select = model_selection.KFold(num_fold)
        fold_accuracies = []
        for train_index, val_index in model_select.split(X):
    ...
    best_lambda = sorted(lam_accuracies)[0]
