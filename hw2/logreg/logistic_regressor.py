...
class LogisticRegressor:
    ...
    def grad_loss(self, *args):
        ...
        grad = np.dot(X.T, utils.sigmoid(np.dot(X, theta)) - y) / (1.0 * m)
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        h = utils.sigmoid(np.dot(X, theta))
        J = (1.0 / m) * (- np.dot(y.T, np.log(h)) - np.dot((1 - y).T, np.log(1 - h))) + reg * np.sum(theta[1:] ** 2) / (2 * m)
    ...
    def grad_loss(self, *args):
        ...
        grad = (1.0 / m) * np.dot(X.T, utils.sigmoid(np.dot(X, theta)) - y) + np.append([0], reg * theta[1:] / m)
